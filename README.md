...
### Spawn Agent
`Conduit: Spawn Agent` - select a model and enter a task. The agent runs in the background with its own output channel. Spawn sends `mode: agent` and the workspace path so the selected CLI can edit files with its own tools.
...
### Agent Tools
Chat **Agent** mode runs tools in the extension and keeps the CLI on `mode: chat` (read-only). Spawn and Fix Issue do not use this table; they send `mode: agent` and the CLI uses its own tools.

Host-side chat Agent tools:
...
| `/plan [task]` | Switch to Plan mode and ask the CLI native planner (`mode: plan`) |
...
- Dev deps: `eslint` major bump 8.x to ^10.1.0
- No runtime changes; closes Dependabot PRs #30 and #34

### v0.7.2 (2026-05-05)
- Security: vite 8.0.5 (path traversal in `.map` handling, GHSA-4w7w-66w2-5vf9)
- Security: esbuild ^0.27.7 (dev server bypass, GHSA-67mh-4wv8-2f99)
- Security: brace-expansion transitive bump (GHSA-f886-m6hf-6m8v)
- Aligned `@typescript-eslint/parser` to ^8.59.2 to match the plugin and unblock CI peer-dep resolution
- Bumped vitest and `@vitest/coverage-v8` to ^4.1.5

### v0.7.1 (2026-03-24)
- Fix: resolve `.cmd` wrapper to actual JS entry on Windows when spawning the bridge
