> Note (2026-09-03, claude-opus-5): Cut v0.10.2. It carries the typecheck work and one user-visible fix that was sitting unreleased on main: SessionsTreeProvider.refresh took string | undefined while its own event emits string | null, so the sessions tree could never clear its highlight once a session ended. First release cut entirely by .github/workflows/release.yml, which builds the .vsix and publishes it as the release asset in one run. version-sync checked all six pinned sites before the tag rather than after, which is the difference from v0.10.0.

> Note (2026-09-03, claude-opus-5): Added npm run typecheck (tsc --noEmit) and wired it into CI before the build. Nothing had ever type-checked this extension: esbuild bundles without checking types, so 159 errors had accumulated while TRUST.md recorded the opposite as tool_verified. CORRECTION to the note written earlier today: the "33 genuine noImplicitAny errors" were not genuine. tsconfig declared no "types", so node globals resolved to nothing and the implicit-any errors were downstream of that; adding "types": ["node"] took 159 to 2. Both remaining errors were real - a dynamic import missing the .js extension Node16 resolution requires, and SessionsTreeProvider.refresh typed string | undefined while its own event emits string | null, so the tree could never clear its highlight. The TRUST row is verified again, with the measurement recorded. Mutation-proved: a deliberate type error turns the gate red. 333 tests pass; the CI job keeps the name "test", so the required status check is unaffected.

> Note (2026-09-03, claude-opus-5): Parity pass against conduit-bridge, both directions. This repo lost the Dependabot exemption on the AAHP gate - the aahp check step added this morning had inherited it, so the gate meant to stop another v0.10.0 was bypassable from birth. CHANGELOG.md moved from docs/ to the repo root, because the changelog and changelog-format gates hardcode that path and had therefore skipped forever; it was grammar-cleaned first (six invalid ### headings merged into Changed, thirteen compare links added, 83 bullets before and after) and all six path couplings moved in the same commit, release.yml included. TRUST.md was asserting two things that measurement contradicts, hidden because every date cell was blank so Layer 4 never evaluated: v0.1.0 and v0.2.0 have ZERO release assets, not a .vsix; and nothing type-checks this repo - tsc --noEmit reports 159 errors, about 120 from tsconfig declaring no types and 33 genuine noImplicitAny errors in src. That row is now unverified rather than quietly fixed, because it is source work. OPEN: the typecheck, and the 84 em dashes in conduit-bridge .ts files (19 in runtime strings).

> Note (2026-09-03, claude-opus-5): Untracked .claude/settings.local.json and added it to .gitignore. It was committed on 2026-03-15 (4aa03c9) and had been public in this repository for five months. It holds no credentials - checked for ghp_/sk-/xox*/AKIA/private-key/token/secret/password patterns, zero matches - but it is a Claude Code permission allowlist of absolute paths, so it published the operator username (4 occurrences) and an internal host and share path (5 occurrences). Nothing in .gitignore caught it, and it was the only tracked file under .claude/. IMPORTANT, and it changes what a history rewrite is worth here: all 75 refs/pull/N/head refs contain both the commit and the file. GitHub creates those refs, they survive merge and branch deletion, and a repository owner cannot delete or rewrite them, so the blob stays publicly fetchable after any force-push of main and tags. Removing it from the pull-request refs and cached views requires a GitHub Support request, or deleting and recreating the repository. The commit itself is on no pull request, so main and the 18 tags are the only owner-controlled refs.

> Note (2026-09-03, claude-opus-5): Added .github/workflows/release.yml. Building the .vsix and publishing the release were two manual acts, and the second was forgettable: v0.1.0, v0.2.0 and v0.6.0 are still empty today, and v0.10.0 was published at 22:35:18Z with its asset uploaded by hand at 22:40:38Z, so the only download link this extension has was dead for five minutes. The workflow builds and publishes in one run and asserts afterwards that the release is not a draft and carries a fully uploaded, non-empty asset whose sha256 is the one the gates passed. It refuses a tag that disagrees with package.json, is not an ancestor of main, or has no matching docs/CHANGELOG.md section. Build and publish are separate jobs: build runs npm ci and vsce with contents: read only, publish holds contents: write and runs no repository or registry code, so nothing from npm can reach the release token. OPERATIONAL: GitHub loads the workflow from the TAGGED COMMIT, so a tag cut at a commit predating this file runs nothing, silently and with no red X. Known residual risks are listed in the file header and in PR #92.
...
## Current Version: 0.10.2 (GitHub .vsix only - Marketplace publishing dropped by decision 2026-07-17; issue #85 locked)
